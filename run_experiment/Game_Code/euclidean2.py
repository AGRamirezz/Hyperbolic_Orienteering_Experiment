...
order = [10,20,12,3,2,13,18,11,16,17]
...
for iterator in range(len(order)):
	...
	df = pd.read_excel('/Users/cogmech/Documents/NRT-Hyperbolic_Search/experiment/experiment/Eucl_Node_Location_Data/'+str(order[iterator])+'.xlsx')
	df = df.as_matrix()
	...
	df2 = pd.read_excel('/Users/cogmech/Documents/NRT-Hyperbolic_Search/experiment/experiment/Eucl_Node_Distances_Data/'+str(order[iterator])+'.xlsx')
	df2 = df2.as_matrix()
	...
	player_fuel = energy*20 
	player_fuel_back = energy*20
	ii = 0
	...
	while not done:
	            ...
	            for j, row in enumerate(df):
	                if df[j][0]-8 <= mouse[0] <= df[j][0]+8 and df[j][1]-8 <= mouse[1] <= df[j][1]+8:
	                    ...
	                    if ii >= 1:          
	                        ...
	                        player_fuel = player_fuel - df2[mindx[ii-1]][mindx[ii]]*20
	    ...
	    if event.type == pygame.QUIT or energy <= 0:
	    	...
	    	os.chdir('/Users/cogmech/Documents/NRT-Hyperbolic_Search/experiment/experiment/game_data/part30/euclidean')
	    	nvisited = [[nx],[ny]]

	    	##outputdata into csv file
	    	eog = np.transpose([timehist,energyhist,nx,ny,node,scorehist])

	    	eogdata = pd.DataFrame(data=eog,columns=['Time','Energy','X','Y','Node','Score'])
	    	eogdata.to_csv('eogdata'+'_'+now+'.csv', sep=',')
	    	done = True