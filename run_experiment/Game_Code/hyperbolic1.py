...
order = [11,17,10,18,7,9,20,6,16,14]
...
for iterator in range(len(order)):
	...
	##Loads gameboard image
	gameboard = pygame.image.load('gameboard.jpg')
	screen.blit(gameboard,(285,15))
	pygame.display.flip()
	##Loads node location data and scales it to proper screen dimensions
	df = pd.read_excel('/Users/cogmech/Documents/NRT-Hyperbolic_Search/experiment/experiment/Node_Location_Data/'+str(order[iterator])+'.xlsx')
	df = df.as_matrix()
	...
	df2 = pd.read_excel('/Users/cogmech/Documents/NRT-Hyperbolic_Search/experiment/experiment/Node_Distances_Data/'+str(order[iterator])+'.xlsx')
	df2 = df2.as_matrix()
	...
	while not done:
	    ...
	    if event.type == pygame.QUIT or energy <= 0:
	    	...
	    	os.chdir('/Users/cogmech/Documents/NRT-Hyperbolic_Search/experiment/experiment/game_data/part30/hyperbolic')
	    	nvisited = [[nx],[ny]]

	    	##outputdata into csv file
	    	hog = np.transpose([timehist,energyhist,nx,ny,node,scorehist])

	    	hogdata = pd.DataFrame(data=hog,columns=['Time','Energy','X','Y','Node','Score'])
	    	hogdata.to_csv('hogdata'+'_'+now+'.csv', sep=',')
	    	done = True